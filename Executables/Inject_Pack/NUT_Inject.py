...
for i in range(fileTotal):
        ...
        if i > 0:
                ...
                offsetHeader += 0x80
        backNTime = nut.tell()
        nut.seek(offsetHeader + 0x4)
        fileGTXwth = readu32be(nut)
        fileGTXhth = readu32be(nut)
        nut.seek(offsetHeader + 0x17)
        fileGTXfmt = readByte(nut)
        ...
        if i == texID:
                ...
                GTX = open("tmp.gtx", 'rb')
                GTX.seek(0x44)
                GTXwth = readu32be(nut)
                GTXhth = readu32be(nut)
                GTX.seek(0x57)
                GTXfmt = readByte(nut)
                GTX.seek(0xFC)
                magic = 0
